      ...
      <UBox direction="row" style="gap: 5px">
        <UItem :ratio="6">
          <WjFlexGrid style="height: 600px" :initialized="gridAction.histGrid.initialize">
            ...
            <WjFlexGridColumn binding="orgCd" header="회사 명" width="*" :cellTemplate="ctx => useUstraCodeValue('ORG_CD', ctx.value)" />
            <WjFlexGridColumn
              :binding="$ustra.management.serverAppProps.useUserDeptCodeSelectBox ? 'deptCd' : 'deptNm'"
              header="부서 명"
              width="*"
              :cellTemplate="ctx => ($ustra.management.serverAppProps.useUserDeptCodeSelectBox ? useUstraCodeValue('DEPT_CD', ctx.value) : ctx.value)"
            />
            <WjFlexGridColumn binding="usrSttCd" header="사용자 상태" :cellTemplate="ctx => useUstraCodeValue('USR_STT_CD', ctx.value)" />
            <WjFlexGridColumn binding="01" header="권한" width="*" />
            <WjFlexGridColumn binding="02" header="기초 설정" width="*" />
            <WjFlexGridColumn binding="99" header="기타" width="*" />
...
<script lang="ts">
export default {
  name: 'UstraManagementSystemUserAuthStt',
}
</script>
